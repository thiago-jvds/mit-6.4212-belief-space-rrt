...
def visualize_belief_tree(rrbt_tree, problem, meshcat, iteration):
    ...
    if iteration == 1:
        ...
        if children_of_root:
            ...
            if len(grandchild) > 0:
                print(f"First child IS Intermediate (or Junction). is_key_node should be False.")
            else:
    ...
    n_leaves = sum(1 for n in key_nodes if sum(1 for x in rrbt_tree.nodes if x.parent == n) == 0)
    n_junctions = len(key_nodes) - n_roots - n_leaves
    
    print(f"\r   > RRBT Iter {iteration}: {len(rrbt_tree.nodes)} nodes -> {len(key_nodes)} keys "
          f"(R:{n_roots}, L:{n_leaves}, J:{n_junctions})", end="", flush=True)
    ...
    for i, node in enumerate(key_nodes):
        ...
        meshcat.SetTransform(f"rrbt_tree/node_{i}", RigidTransform(RotationMatrix(), pos))
    ...
    for node in key_nodes:
        ...
        if curr is not None:
            ...
            in_light = problem.is_in_light(node.value)
            edge_color = Rgba(0.5, 1.0, 0.5, 0.7) if in_light else Rgba(1.0, 0.5, 0.5, 0.7)
            meshcat.SetLine(f"rrbt_tree/edge_{edge_idx}", 
                           np.column_stack([start_pos, end_pos]), rgba=edge_color)
            edge_idx += 1
